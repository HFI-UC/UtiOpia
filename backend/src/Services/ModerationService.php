    ...
    public function approve(int $messageId, array $actor): array
    {
        ...
        if ($row = $stmt->fetch()) {
            if ((int)$row['is_anonymous'] === 1 && !empty($row['anon_email'])) {
                $this->mailer->sendMessageRestored((string)$row['anon_email'], '同学', $messageId, (string)$row['content']);
            } else if (!empty($row['email'])) {
                $this->mailer->sendMessageRestored((string)$row['email'], (string)$row['nickname'], $messageId, (string)$row['content']);
            }
        }
        return ['ok' => true];
    }
